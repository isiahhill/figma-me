...
// from a synthetic POC threshold at a glance, and that distinction is the
...
// The preview is the artifact a non-engineer actually looks at, so the badge
// says what happened in words. The raw status stays available in decision.json
// for anyone who wants the machine value.
const GATE_LABELS: Record<string, string> = {
  ready_for_cio: "Ready to hand off",
  review: "Needs a human read",
  blocked: "Stopped",
};
const LINT_LABELS: Record<string, string> = {
  pass: "Brand check passed",
  warn: "Brand check passed with warnings",
  fail: "Brand check failed",
};
...
<body style="margin:0;font-family:Helvetica,Arial,sans-serif;background:#f3f4f6;">
  <div style="padding:16px 24px;background:#111827;color:#fff;">
    ...
    <span style="margin-left:16px;padding:2px 10px;border-radius:99px;background:${GATE_COLORS[gate]};font-size:12px;">${GATE_LABELS[gate] ?? gate}</span>
...
${emailPane}
    ...
    <aside style="flex:1;min-width:260px;">
      <h2 style="font-size:14px;text-transform:uppercase;letter-spacing:0.05em;color:#6b7280;">${LINT_LABELS[draft.lint.status] ?? draft.lint.status}</h2>
${rows}
    </aside>
  </div>
</body>
